...
<body class="bg-gray-100 min-h-screen">
    <div class="container mx-auto p-4 max-w-4xl">
        ...
        @endif
        <form method="POST" action="{{ route('habit.documents.store', ['devis_id' => $devis_id]) }}" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md" x-data="{ selectedAgence: {{ session('habitation_data.agence_id', 'null') }} }">
            @csrf
            <h2 class="text-xl font-semibold mb-4">Télécharger les documents (images uniquement : JPG, PNG)</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    <label class="block text-gray-700 font-medium mb-2" for="titre_foncier">Titre Foncier</label>
                    <input type="file" name="titre_foncier" accept="image/jpeg,image/png" required class="w-full border rounded p-2">
                    @error('titre_foncier') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
                <div>
